export async function addItemMiddleware(
    ...
    try {
      ...
      if (expected) {
        ...
        console.log("500 error pako", error)
      ctx.status = 500
      ctx.body = error
      await next()
    }
    ...
    async function addItem(
      ...
      try {
        ...
        const updateCollectionClientResponse = await collectionClient.addItem(
          subCollectionId,
          {"SkuId": updateRequest.sku}
        )
  
        const addItemMiddlewareResponse: UpdateResponse = {
          sku: updateRequest.sku,
          success: updateCollectionClientResponse.data,
          subCollectionId: updateRequest.subCollectionId
        }
  
        return addItemMiddlewareResponse
      } catch (error)
      
      { console.log("paco:", error)
